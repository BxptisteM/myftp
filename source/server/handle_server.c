...
void run_ftp(server_t *server)
{
    ...
    while (1) {
        setup_file_descriptors(server->server_socket->fd, &server->readfds, &server->max_fd, server->clients);
        activity = select(server->max_fd + 1, &server->readfds, NULL, NULL, NULL);
